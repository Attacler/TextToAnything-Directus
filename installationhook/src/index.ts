    ...
    ({ action }, { services, getSchema, logger, env, emitter }) => {
        getSchema().then(async (schema) => {
            ...
            for (const field of [
                ...
                if (
                    schema.collections.directus_settings?.fields[field] == null
                ) {
                    await addField(
                        field,
                        "string",
            ...
            if (
                schema.collections.directus_settings?.fields
                    .TTA_VIRUSSCANNER_ENABLED == null
            )
                await addField(
            ...
            if (
                schema.collections.directus_folders?.fields
                    .TTA_VIRUSSCAN_ENABLED == null
            )
                await addField(
            ...
            (globalThis as any).TTA = {
                async generateBarCode(context: any) {
                    ...
                    return await fileSerice.uploadOne(download.data, {
                        filename_download:
                            "barcode-" + context.barcodeContent + ".gif",
                ...
                async generatePDF(context: any) {
                    ...
                    if (
                        Object.keys(context.templatevariables || {}).length > 0
                    ) {
                        const header = engine.parse(
                            context.pdfoptions.header || "",
                        );
                        const tpl = engine.parse(context.pdfoptions.html);
                        const footer = engine.parse(
                            context.pdfoptions.footer || "",
                        );
                ...
                async generatePDFFromTemplate(context: any) {
                    ...
                    const template = await templateService.readOne(
                        context.template,
                    );
                ...
                async generateQRCode(context: any) {
                    ...
                    return await fileSerice.uploadOne(download.data, {
                        filename_download:
                            "qrcode-" + context.qrcodeContent + ".png",
                ...
                async virusscanAsset(fileID: string) {
                    ...
                    const { stream, file } =
                        await assetsService.getAsset(fileID);
                    ...
                    if (buffer.byteLength > 50000000) {
                        logger.warn(
                            "[TTA] " +
                                fileID +
                                " has been ignored because of the file 50MB file limit.",
                        );
                        return;
                    }
